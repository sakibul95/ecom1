...
type DeliveryZone = "inside-dhaka" | "outside-dhaka" | "outside-district"
...
export function OrderForm() {
  ...
  // Product selection state: which product is selected + quantities
  const [selectedProduct, setSelectedProduct] = useState<string>(products[0]?.id ?? "")
  const [quantities, setQuantities] = useState<Record<string, number>>(() =>
    Object.fromEntries(products.map((p) => [p.id, p.defaultQty]))
  )
  ...
  const subtotal = activeProduct.price * activeQty
  const deliveryCharge =
    deliveryOptions.find((d) => d.id === delivery)?.charge ?? 50
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="bg-card rounded-2xl border border-border p-8 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button
                type="submit"
                className="w-full bg-primary text-primary-foreground rounded-lg py-4 text-base font-semibold hover:bg-primary/90 active:scale-[0.99] transition-all mt-2"
              >
                অর্ডার কনফার্ম করুন {total.toLocaleString("bn-BD")} TK
              </button>
          ...
          <div className="space-y-4">
            <h2 className="font-serif text-2xl text-white">পণ্য বাছাই করুন</h2>
